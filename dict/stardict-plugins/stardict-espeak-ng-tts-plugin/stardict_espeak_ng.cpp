...
{
	return build_path(gpAppDirs->get_user_config_dir(), "espeak-ng.cfg");
}
...
static void on_tts_combobox_changed(GtkComboBox *widget, gpointer data)
{
	...
	gchar *data1 = g_strdup_printf("[espeak-ng]\nvoice=%s\n", voice_engine.c_str());
...
{
	GtkWidget *window = gtk_dialog_new_with_buttons(_("Espeak-ng TTS configuration"), GTK_WINDOW(plugin_info->pluginwin), GTK_DIALOG_MODAL, GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, NULL);
...
{
	g_debug(_("Loading Espeak-ng plug-in..."));
	if (strcmp(obj->version_str, PLUGIN_SYSTEM_VERSION)!=0) {
		g_print(_("Error: Espeak-ng plugin version doesn't match!\n"));
		return true;
	}
	obj->type = StarDictPlugInType_TTS;
	obj->info_xml = g_strdup_printf("<plugin_info><name>%s</name><version>1.0</version><short_desc>%s</short_desc><long_desc>%s</long_desc><author>Hu Zheng &lt;huzheng001@gmail.com&gt;</author><website>http://stardict-4.sourceforge.net</website></plugin_info>", _("Espeak-ng"), _("Espeak-ng TTS."), _("Pronounce words by Espeak-ng TTS engine."));
...
bool stardict_tts_plugin_init(StarDictTtsPlugInObject *obj)
{
	...
	if (!g_file_test(res.c_str(), G_FILE_TEST_EXISTS)) {
		g_file_set_contents(res.c_str(), "[espeak-ng]\nvoice=\n", -1, NULL);
	}
	GKeyFile *keyfile = g_key_file_new();
	g_key_file_load_from_file(keyfile, res.c_str(), G_KEY_FILE_NONE, NULL);
	gchar *str = g_key_file_get_string(keyfile, "espeak-ng", "voice", NULL);
