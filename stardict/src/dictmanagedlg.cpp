...
enum NetworkAddDlgDictColumns {
	...
	NETWORK_ADD_DLG_NEED_LEVEL_COLUMN, // 4
...
static void dirinfo_parse_end_element(GMarkupParseContext *context, const gchar *element_name, gpointer user_data, GError **error)
{
	if (strcmp(element_name, "dir")==0) {
		...
		gtk_tree_store_set(
			...
			NETWORK_ADD_DLG_VISIBLE_COLUMN, TRUE,
			NETWORK_ADD_DLG_WORD_COUNT_COLUMN, atol(Data->dict_wordcount.c_str()),
...
static void create_dict_item_model(GtkTreeStore *model, GtkTreeIter *group_iter, std::list<DictManageItem> &dictitem, bool is_query)
{
	...
	if (is_query) {
		gtk_tree_store_set(
			...
			DICT_MANAGE_ROW_LEVEL_COLUMN, 1,
		...
		if (i->type == LOCAL_DICT) {
			...
			if (dictinfo.load_from_ifo_file(i->file_or_id.c_str(),
				...
				gtk_tree_store_set(
					...
					DICT_MANAGE_WORD_COUNT_COLUMN, dictinfo.get_wordcount(),
					...
					DICT_MANAGE_ROW_LEVEL_COLUMN, 2,
					DICT_MANAGE_EDITABLE_COLUMN, FALSE,
					DICT_MANAGE_TYPE_COLUMN, LOCAL_DICT,
					-1
				);
				g_free(markup);
			}
			...
			if (gpAppFrame->oStarDictPlugins->VirtualDictPlugins.find_dict_by_id(i->file_or_id.c_str(), iPlugin)) {
				...
				gtk_tree_store_set(
					...
					DICT_MANAGE_WORD_COUNT_COLUMN, 0,
					...
					DICT_MANAGE_ROW_LEVEL_COLUMN, 2,
					DICT_MANAGE_EDITABLE_COLUMN, FALSE,
					DICT_MANAGE_TYPE_COLUMN, VIRTUAL_DICT,
					-1
				);
			}
			...
			if (gpAppFrame->oStarDictPlugins->NetDictPlugins.find_dict_by_id(i->file_or_id.c_str(), iPlugin)) {
				...
				gtk_tree_store_set(
					...
					DICT_MANAGE_ROW_LEVEL_COLUMN, 2,
					DICT_MANAGE_EDITABLE_COLUMN, FALSE,
					DICT_MANAGE_TYPE_COLUMN, NET_DICT,
					-1
				);
			}
		}
...
GtkTreeModel* DictManageDlg::create_dictmanage_tree_model()
{
	...
	for (std::list<DictManageGroup>::iterator i = gpAppFrame->dictinfo.groups.begin(); i != gpAppFrame->dictinfo.groups.end(); ++i) {
		...
		gtk_tree_store_set(
			...
			DICT_MANAGE_SHOW_DETAILS_COLUMN, false,
			DICT_MANAGE_ROW_LEVEL_COLUMN, 0,
		...
		if (dictinfo.load_from_ifo_file(url.c_str(),
			...
			if(istreedict) {
				gtk_list_store_set(
					...
					TREEDICT_WORD_COUNT_COLUMN, dictinfo.get_wordcount(),
				...
				gtk_list_store_set(
					model, &iter,
					DICTLIST_TYPE_COLUMN, LOCAL_DICT,
					DICTLIST_NAME_COLUMN, dictinfo.get_bookname().c_str(),
					DICTLIST_WORD_COUNT_COLUMN, dictinfo.get_wordcount(),
...
GtkTreeModel* DictManageDlg::create_tree_model(TDictTree dicttree)
{
	...
	if (dicttree == DictTree_TreeDict) {
		...
		for (size_t i = 0; i < n; i++) {
			...
			gtk_list_store_set(
				model, &iter,
				DICTLIST_TYPE_COLUMN, VIRTUAL_DICT,
				DICTLIST_NAME_COLUMN, dictname,
				DICTLIST_WORD_COUNT_COLUMN, 0,
...
static void process_dictmanage_type_iter(GtkTreeModel *model, GtkTreeIter *parent_iter, std::string &configxml)
{
	...
	while (have_next) {
		...
		gtk_tree_model_get (
			...
			DICT_MANAGE_TYPE_COLUMN, &dicttype,
			-1
		);
...
void DictManageDlg::SaveDictManageList()
{
	...
	while (have_iter) {
		gtk_tree_model_get (dict_list_tree_model, &iter, DICTLIST_TYPE_COLUMN, &dicttype, -1);
		if (dicttype == LOCAL_DICT) {
...
static void dictmask_parse_end_element(GMarkupParseContext *context, const gchar *element_name, gpointer user_data, GError **error)
{
	if (strcmp(element_name, "dict")==0) {
		...
		gtk_list_store_set(
			...
			NETWORKDICT_WORD_COUNT_COLUMN, atol(Data->wordcount.c_str()),
			-1
		);
	}
}
