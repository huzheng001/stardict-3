...
int textual_dict_gen_t::prepare_output_files(void)
{
	...
	if(!xmlfilemain) {
		print_info(open_write_file_err, xmlfilenamemain.c_str());
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
...
int textual_dict_gen_t::generate_header(void)
{
	if(0 > xmlfilemain_puts(UTF8_BOM
		...
		print_info(write_file_err, xmlfilenamemain.c_str());
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
...
int textual_dict_gen_t::generate_article(const article_data_t& article)
{
	...
	for(size_t i=0; i<article.definitions.size(); ++i) {
		...
		if(g_ascii_islower(type_id)) {
			...
			print_info("Index item %s, type = '%c'. Binary data types are not supported presently. Skipping.\n",
...
int textual_dict_gen_t::prepare_chunk_file(void)
{
	...
	if(!xmlfilechunk) {
		print_info(open_write_file_err, chuck_file_name.c_str());
		return EXIT_FAILURE;
	}
	return EXIT_SUCCESS;
}
		...
		size_t chunk_size, print_info_t print_info)
{
	...
	if(parser.load(ifofilename, print_info)) {
		print_info("Dictionary load failed.\n");
		return EXIT_FAILURE;
	}
	print_info("Dictionary loaded.\n");
	common_dict_t parsed_norm_dict;
	parsed_norm_dict.set_print_info(print_info);
	if(convert_to_parsed_dict(parsed_norm_dict, parser, print_info))
		return EXIT_FAILURE;
	print_info("Generating xml file...\n");
	...
	if(generator.generate(xmlfilename, &parsed_norm_dict)) {
		print_info("Generating xml file failed.\n");
		return EXIT_FAILURE;
	}
	print_info("xml file's created.\n");
	return EXIT_SUCCESS;
}
